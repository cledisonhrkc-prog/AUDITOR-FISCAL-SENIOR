...
export default function App() {
  ...
  const handleAnalyzeWithAI = async (batchId: string) => {
    ...
    try {
      ...
      saveBatches(updatedBatches);
      // GRAVACAO AUTOMATICA no Supabase apos auditoria (fail-safe, nao trava o app)
      const loteAuditado = updatedBatches.find(b => b.id === batchId);
      if (loteAuditado) {
        salvarLoteNoSupabase(loteAuditado).then(r => {
          if (r.ok) console.log("Lote gravado no Supabase:", r.gravados, "notas");
          else console.warn("Gravacao Supabase falhou:", r.erro);
        });
      }
    } catch (err: any) {
      console.error(err);
                ...
                <div className="lg:col-span-2 space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6" id="context-tax-widgets">
                    ...
                    <div className={`bg-gradient-to-br from-slate-900 to-slate-950 text-white rounded-3xl p-6 border border-slate-800 shadow-md flex flex-col justify-between space-y-4 ${activeBatch.clientRegime === 'MEI' ? 'col-span-2 md:col-span-1' : 'col-span-2'}`} id="reform-2027-widget">
                      ...
                      <p className="text-[10px] text-amber-400 bg-amber-500/10 border border-amber-500/30 rounded px-2 py-1 mt-1">? PROJE��O: al�quotas CBS 8,8% e IBS 17,7% s�o estimativas de refer�ncia (Minist�rio da Fazenda / LC 214/2025), ainda n�o fixadas em lei definitiva. Valores sujeitos a altera��o. Simula��o meramente indicativa.</p>
...
<ReportSections batch={activeBatch} />
                ...
                <div className="py-6 space-y-6 text-slate-800 text-xs leading-relaxed" id="document-ai-body">
                  ...
                  {activeBatch.aiAnalysis ? (
                    <div className="space-y-6">
                      <div className="text-[10px] text-amber-700 bg-amber-50 border border-amber-300 rounded px-3 py-2 mb-2">? AN�LISE COMPLEMENTAR POR IA: esta se��o � gerada por intelig�ncia artificial e pode conter imprecis�es. O parecer t�cnico com fundamenta��o legal verificada � o das se��es anteriores (motor determin�stico). Confira sempre antes de agir.</div>
